...
test("Diagramme, Dialoge und lange Formulare passen auch bei Touch und Drehung", async ({ page }, testInfo) => {
  ...
  const abmessungen = page.getByRole("region", { name: "Interaktive Darstellung der Wundabmessungen" });
  await expect(abmessungen).toBeVisible();
  const zeitstrahl = abmessungen.getByRole("group", { name: "Aufnahme auswählen" });
  const slider = zeitstrahl.getByRole("slider");
  await expect(zeitstrahl.locator("time")).toHaveCount(3);
  await slider.scrollIntoViewIfNeeded();
  const sliderBox = (await slider.boundingBox())!;
  expect(sliderBox.height).toBeGreaterThanOrEqual(44);
  const sliderPosition = { x: 14, y: sliderBox.height / 2 };
  if (testInfo.project.use.hasTouch) await slider.tap({ position: sliderPosition });
  else await slider.click({ position: sliderPosition });
  await expect(slider).toHaveValue("1");
  await expect(zeitstrahl.locator("[data-ausgewaehlter-termin]")).toHaveAttribute("datetime", (await zeitstrahl.locator("time").nth(1).getAttribute("datetime"))!);
