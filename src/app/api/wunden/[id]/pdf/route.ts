...
import { erzeugeVerlaufPdf, PdfExportFehler } from "@/lib/pdf/export";
...
export async function GET(
  ...
  try {
    const { bytes, dateiname, anzahlAufnahmen } = await erzeugeVerlaufPdf(id);
    await protokolliere(sitzung.user.id, "Wound", id, "PDF_EXPORT", `Verlauf: ${anzahlAufnahmen} Aufnahmen`);
